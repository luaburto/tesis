...
data1['SEMANA'] <- strftime(data[,1], format = "%V")
data1['MONTOVENTA_UNIDAD'] <- data1['F_MONTOVENTA']/data1['F_UNIDADESVENTA']
data1['COSTOVENTA_UNIDAD'] <- data1['F_COSTOVENTA']/data1['F_UNIDADESVENTA']
...
data3 <- data2 %>% group_by(SEMANA,DESC_LOCALFISICO,CLASE,COD_SKU, DESC_SKU)  %>%
  summarise(TOTAl_F_UNIDADESVENTA = sum(F_UNIDADESVENTA),
            ...
            VENTA_PROMEDIO = sum(F_MONTOVENTA)/sum(F_UNIDADESVENTA),
            VENTA_PROMEDIO2 = mean(MONTOVENTA_UNIDAD),
            TOTAl_F_COSTOVENTA = sum(F_COSTOVENTA),
            COSTOVENTA_PROMEDIO = sum(F_COSTOVENTA)/sum(F_UNIDADESVENTA),
            .groups = 'drop')


clases_global <- distinct(data3[,c("CLASE")])
locales <- distinct(data3[,c("COD_LOCALFISICO")])

#datos de determinada clase de todos los locales
data_global <- data3 %>%
  filter( CLASE==clases_global[4,]) #Elaborados
...
for (k in 1:n_locales){
  ...
  data_local <- data_global %>%
    filter(COD_LOCALFISICO==locales[k,])
  ...
  for (i in 1:n_weeks){
    for (j in 1:n_skus){
      consulta <- data_local %>%
        filter(COD_SKU==skus[j,], SEMANA==semanas[i,])
  ...
  for (i in 1:n_skus){
    for (j in 1:n_weeks){
      consulta2 <- data_local %>%
        filter(COD_SKU==skus[i,], SEMANA==semanas[j,])
...
nombres_columnas = c("COD_LOCALFISICO","COD_SKU","DEMANDA","SEMANA")
p_sku_nombres <- c()
for (sku in skus){
...
write.csv(etl_completa,"/Users/joseizammontt/Desktop/Universidad/master/thesis/codigo/Elavorados.csv", row.names = FALSE)
